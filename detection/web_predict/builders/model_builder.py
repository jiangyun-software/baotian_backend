...
#build model by frontend which is the base feature extractor in classify tool
def build_classify_model(net_input, num_classes, frontend="MobileNetV2", is_training=True):
	...
	network = None
	init_fn = None
	logits, end_points, frontend_scope, init_fn  = frontend_builder.build_frontend(net_input, frontend, is_training=is_training)

	network = slim.conv2d(logits, num_classes, [1, 1], scope='logits')
	return network, init_fn

# build_model by model and frontends in segment tool
def build_model(model_name, net_input, num_classes, crop_width, crop_height, frontend="MobileNetV2", is_training=True):
