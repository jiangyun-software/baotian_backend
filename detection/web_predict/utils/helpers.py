...
# acquire label map colors of segement tool
...
#acquire label infos for classify tools, label template filename,label
def get_classify_label_info(csv_path):
    """
    Retrieve the class names and label values for the selected dataset.
    Must be in CSV format!
    ...
    with open(csv_path, 'r') as csvfile:
        ...
        header = next(file_reader)
        for row in file_reader:
            data_file_names.append(row[0])
            label_values.append(int(row[1]))
        # print(class_dict)
    return data_file_names, label_values
